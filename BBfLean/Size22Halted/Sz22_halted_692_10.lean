...
/-!
# sz22_halted_692 #10: [9/10, 343/2, 4/21, 5/7, 14/5]
...
private theorem r2_drain_sn : ∀ k a d, (⟨a + k, 0, 0, d⟩ : Q) [fm]⊢^{k} ⟨a, 0, 0, d + 3 * k⟩ := by
...
private theorem r3r2_drain_sn : ∀ k b d, (⟨0, b + k, 0, d + 1⟩ : Q) [fm]⊢^{3 * k} ⟨0, b, 0, d + 1 + 5 * k⟩ := by
  intro k; induction' k with k ih <;> intro b d
  · simp; rfl
  rw [show b + (k + 1) = (b + k) + 1 from by ring, show 3 * (k + 1) = 3 + 3 * k from by ring]
  apply stepNat_trans 3 (3 * k)
  · rw [show (3 : ℕ) = 1 + (1 + 1) from by ring]
    apply stepNat_trans 1 _
    · show fm ⟨0, (b + k) + 1, 0, d + 1⟩ = some ⟨2, b + k, 0, d⟩; simp [fm]
    apply stepNat_trans 1 1
    · show fm ⟨2, b + k, 0, d⟩ = some ⟨1, b + k, 0, d + 3⟩; simp [fm]
    show fm ⟨1, b + k, 0, d + 3⟩ = some ⟨0, b + k, 0, d + 6⟩; simp [fm]
  rw [show d + 1 + 5 * (k + 1) = (d + 5) + 1 + 5 * k from by ring,
    show d + 6 = (d + 5) + 1 from by ring]
  exact ih b (d + 5)
...
private theorem boundary_r1_sn : ∀ b, (⟨0, b + 1, 1, 0⟩ : Q) [fm]⊢^{3 * b + 5} ⟨0, 0, 0, 5 * b + 9⟩ := by
  intro b
  rw [show 3 * b + 5 = 1 + (1 + 3 * (b + 1)) from by ring]
  apply stepNat_trans 1 _
  · show fm ⟨0, b + 1, 1, 0⟩ = some ⟨1, b + 1, 0, 1⟩; simp [fm]
  apply stepNat_trans 1 (3 * (b + 1))
  · show fm ⟨1, b + 1, 0, 1⟩ = some ⟨0, b + 1, 0, 4⟩; simp [fm]
  have h := r3r2_drain_sn (b + 1) 0 3
  rw [show 0 + (b + 1) = b + 1 from by ring,
    show (3 + 1 : ℕ) = 4 from by ring,
    show 3 + 1 + 5 * (b + 1) = 5 * b + 9 from by ring] at h; exact h

private theorem full_drain_r3 : ∀ b, (⟨1, b, 0, 0⟩ : Q) [fm]⊢^{1 + 3 * b} ⟨0, 0, 0, 3 + 5 * b⟩ := by
  intro b
  apply stepNat_trans 1 (3 * b)
  · show fm ⟨1, b, 0, 0⟩ = some ⟨0, b, 0, 3⟩; simp [fm]
  rcases b with _ | b
  · simp; rfl
  have h := r3r2_drain_sn (b + 1) 0 2
  rw [show 0 + (b + 1) = b + 1 from by ring,
    show (2 + 1 : ℕ) = 3 from by ring,
    show 2 + 1 + 5 * (b + 1) = 3 + 5 * (b + 1) from by ring] at h; exact h

private theorem full_drain_r2 : ∀ b, (⟨2, b, 0, 0⟩ : Q) [fm]⊢^{2 + 3 * b} ⟨0, 0, 0, 6 + 5 * b⟩ := by
  intro b
  apply stepNat_trans 2 (3 * b)
  · rw [show (2 : ℕ) = 1 + 1 from by ring]
    apply stepNat_trans 1 1
    · show fm ⟨2, b, 0, 0⟩ = some ⟨1, b, 0, 3⟩; simp [fm]
    show fm ⟨1, b, 0, 3⟩ = some ⟨0, b, 0, 6⟩; simp [fm]
  rcases b with _ | b
  · simp; rfl
  have h := r3r2_drain_sn (b + 1) 0 5
  rw [show 0 + (b + 1) = b + 1 from by ring,
    show (5 + 1 : ℕ) = 6 from by ring,
    show 5 + 1 + 5 * (b + 1) = 6 + 5 * (b + 1) from by ring] at h; exact h

private theorem macro_r3_sn : ∀ m, (⟨0, 0, 0, 12 * m + 3⟩ : Q) [fm]⊢^{72 * m + 17} ⟨0, 0, 0, 75 * m + 18⟩ := by
  intro m
  rw [show 12 * m + 3 = 0 + (12 * m + 3) from by ring,
    show 72 * m + 17 = (12 * m + 3) + (15 * m + (4 + (1 + 3 * (15 * m + 3)))) from by ring]
  apply stepNat_trans (12 * m + 3) _ (r4_sn (12 * m + 3) 0)
  rw [show (12 * m + 3 : ℕ) = 3 + 4 * (3 * m) from by ring]
  rw [show 15 * m + (4 + (1 + 3 * (15 * m + 3))) = 5 * (3 * m) + (4 + (1 + 3 * (15 * m + 3))) from by ring]
  ...
  rw [show 15 * m + 3 = 15 * m + 3 from rfl]
  have h := full_drain_r3 (15 * m + 3)
  rw [show 3 + 5 * (15 * m + 3) = 75 * m + 18 from by ring] at h; exact h

private theorem macro_r2_sn : ∀ m, (⟨0, 0, 0, 12 * m + 6⟩ : Q) [fm]⊢^{72 * m + 34} ⟨0, 0, 0, 75 * m + 36⟩ := by
  intro m
  rw [show 12 * m + 6 = 0 + (12 * m + 6) from by ring,
    show 72 * m + 34 = (12 * m + 6) + (15 * m + 5 + (3 + (2 + 3 * (15 * m + 6)))) from by ring]
  apply stepNat_trans (12 * m + 6) _ (r4_sn (12 * m + 6) 0)
  rw [show (12 * m + 6 : ℕ) = 2 + 4 * (3 * m + 1) from by ring]
  rw [show 15 * m + 5 + (3 + (2 + 3 * (15 * m + 6))) = 5 * (3 * m + 1) + (3 + (2 + 3 * (15 * m + 6))) from by ring]
...
private theorem macro_r1_sn : ∀ m, (⟨0, 0, 0, 12 * m + 9⟩ : Q) [fm]⊢^{72 * m + 51} ⟨0, 0, 0, 75 * m + 54⟩ := by
  ...
  have h := boundary_r1_sn (15 * m + 9)
  rw [show 3 * (15 * m + 9) + 5 = 3 * (15 * m + 9) + 5 from rfl,
    show 5 * (15 * m + 9) + 9 = 75 * m + 54 from by ring] at h; exact h
