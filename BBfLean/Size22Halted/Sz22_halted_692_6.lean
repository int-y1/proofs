...
/-!
# sz22_halted_692 #6: [4/15, 1/18, 21/2, 11/3, 5/7, 2/11]
...
private theorem r3r1_sn : ∀ k, ∀ a c d e, (⟨a + 1, 0, c + k, d, e⟩ : Q) [fm]⊢^{2 * k} ⟨a + k + 1, 0, c, d + k, e⟩ := by
  ...
  · show stepNat fm 2 ⟨a + 1, 0, (c + k) + 1, d, e⟩ = some ⟨a + 2, 0, c + k, d + 1, e⟩; rfl
