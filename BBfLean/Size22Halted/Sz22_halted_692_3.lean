...
/-!
# sz22_halted_692 #3: [1/45, 25/21, 33/5, 2/3, 7/11, 5/2]
...
private theorem r23_sn : ∀ k, ∀ a c d e, (⟨a, 1, c, d + k, e⟩ : Q) [fm]⊢^{2 * k} ⟨a, 1, c + k, d, e + k⟩ := by
  intro k; induction' k with k ih <;> intro a c d e
  · simp; rfl
  rw [show d + (k + 1) = (d + k) + 1 from by ring, show 2 * (k + 1) = 2 + 2 * k from by ring]
  apply stepNat_trans 2 (2 * k)
  · show stepNat fm 2 ⟨a, 1, c, (d + k) + 1, e⟩ = some ⟨a, 1, c + 1, d + k, e + 1⟩; rfl
  have h := ih a (c + 1) d (e + 1)
  rw [show (c + 1) + k = c + (k + 1) from by ring,
    show (e + 1) + k = e + (k + 1) from by ring] at h
  exact h

private theorem drain_sn : ∀ k, ∀ a c e, (⟨a, 1, c + 3 * k, 0, e⟩ : Q) [fm]⊢^{3 * k} ⟨a, 1, c, 0, e + 2 * k⟩ := by
  intro k; induction' k with k ih <;> intro a c e
  · simp; rfl
  rw [show c + 3 * (k + 1) = (c + 3 * k) + 3 from by ring, show 3 * (k + 1) = 3 + 3 * k from by ring]
  apply stepNat_trans 3 (3 * k)
  · show stepNat fm 3 ⟨a, 1, (c + 3 * k) + 3, 0, e⟩ = some ⟨a, 1, c + 3 * k, 0, e + 2⟩; rfl
  have h := ih a c (e + 2); rw [show (e + 2) + 2 * k = e + 2 * (k + 1) from by ring] at h; exact h

private theorem tail0_sn : ∀ a e, (⟨a, 1, 0, 0, e⟩ : Q) [fm]⊢^{1} ⟨a + 1, 0, 0, 0, e⟩ := by
  intro a e; rfl

private theorem tail1_sn : ∀ a e, (⟨a, 1, 1, 0, e⟩ : Q) [fm]⊢^{3} ⟨a + 2, 0, 0, 0, e + 1⟩ := by
  intro a e; rfl

private theorem tail2_sn : ∀ a e, (⟨a, 1, 2, 0, e⟩ : Q) [fm]⊢^{2} ⟨a, 0, 0, 0, e + 1⟩ := by
  intro a e; rfl
...
private theorem mod0_sn : ∀ k A, (⟨A + 1, 0, 0, 3 * k, 0⟩ : Q) [fm]⊢^{14 * k + 4} ⟨A + 1, 0, 0, 5 * k + 1, 0⟩ := by
  intro k A
  have h1 : (⟨A + 1, 0, 0, 3 * k, 0⟩ : Q) [fm]⊢^{2} ⟨A, 1, 0, 3 * k, 1⟩ := rfl
  have h2 := r23_sn (3 * k) A 0 0 1
  simp only [Nat.zero_add] at h2
  rw [show 1 + 3 * k = 3 * k + 1 from by ring] at h2
  have h3 := drain_sn k A 0 (3 * k + 1)
  simp only [Nat.zero_add] at h3
  rw [show 3 * k + 1 + 2 * k = 5 * k + 1 from by ring] at h3
  have h4 := tail0_sn A (5 * k + 1)
  have h5 := r5_sn (5 * k + 1) (A + 1) 0 0
  simp only [Nat.zero_add] at h5
  have h12 := stepNat_trans 2 (2 * (3 * k)) h1 h2
  have h123 := stepNat_trans (2 + 2 * (3 * k)) (3 * k) h12 h3
  have h1234 := stepNat_trans (2 + 2 * (3 * k) + 3 * k) 1 h123 h4
  have h12345 := stepNat_trans (2 + 2 * (3 * k) + 3 * k + 1) (5 * k + 1) h1234 h5
  rw [show 2 + 2 * (3 * k) + 3 * k + 1 + (5 * k + 1) = 14 * k + 4 from by ring] at h12345
  exact h12345

private theorem mod1_sn : ∀ k A, (⟨A + 1, 0, 0, 3 * k + 1, 0⟩ : Q) [fm]⊢^{14 * k + 10} ⟨A + 2, 0, 0, 5 * k + 3, 0⟩ := by
  intro k A
  have h1 : (⟨A + 1, 0, 0, 3 * k + 1, 0⟩ : Q) [fm]⊢^{2} ⟨A, 1, 0, 3 * k + 1, 1⟩ := rfl
  have h2 := r23_sn (3 * k + 1) A 0 0 1
  simp only [Nat.zero_add] at h2
  rw [show 1 + (3 * k + 1) = 3 * k + 2 from by ring] at h2
  have h3 := drain_sn k A 1 (3 * k + 2)
  rw [show 1 + 3 * k = 3 * k + 1 from by ring,
    show 3 * k + 2 + 2 * k = 5 * k + 2 from by ring] at h3
  have h4 := tail1_sn A (5 * k + 2)
  rw [show 5 * k + 2 + 1 = 5 * k + 3 from by ring] at h4
  have h5 := r5_sn (5 * k + 3) (A + 2) 0 0
  simp only [Nat.zero_add] at h5
  have h12 := stepNat_trans 2 (2 * (3 * k + 1)) h1 h2
  have h123 := stepNat_trans (2 + 2 * (3 * k + 1)) (3 * k) h12 h3
  have h1234 := stepNat_trans (2 + 2 * (3 * k + 1) + 3 * k) 3 h123 h4
  have h12345 := stepNat_trans (2 + 2 * (3 * k + 1) + 3 * k + 3) (5 * k + 3) h1234 h5
  rw [show 2 + 2 * (3 * k + 1) + 3 * k + 3 + (5 * k + 3) = 14 * k + 10 from by ring] at h12345
  exact h12345

private theorem mod2_sn : ∀ k A, (⟨A + 1, 0, 0, 3 * k + 2, 0⟩ : Q) [fm]⊢^{14 * k + 12} ⟨A, 0, 0, 5 * k + 4, 0⟩ := by
  intro k A
  have h1 : (⟨A + 1, 0, 0, 3 * k + 2, 0⟩ : Q) [fm]⊢^{2} ⟨A, 1, 0, 3 * k + 2, 1⟩ := rfl
  have h2 := r23_sn (3 * k + 2) A 0 0 1
  simp only [Nat.zero_add] at h2
  rw [show 1 + (3 * k + 2) = 3 * (k + 1) from by ring] at h2
  have h3 := drain_sn k A 2 (3 * (k + 1))
  rw [show 2 + 3 * k = 3 * k + 2 from by ring,
    show 3 * (k + 1) + 2 * k = 5 * k + 3 from by ring] at h3
  have h4 := tail2_sn A (5 * k + 3)
  rw [show 5 * k + 3 + 1 = 5 * k + 4 from by ring] at h4
  have h5 := r5_sn (5 * k + 4) A 0 0
  simp only [Nat.zero_add] at h5
  have h12 := stepNat_trans 2 (2 * (3 * k + 2)) h1 h2
  have h123 := stepNat_trans (2 + 2 * (3 * k + 2)) (3 * k) h12 h3
  have h1234 := stepNat_trans (2 + 2 * (3 * k + 2) + 3 * k) 2 h123 h4
  have h12345 := stepNat_trans (2 + 2 * (3 * k + 2) + 3 * k + 2) (5 * k + 4) h1234 h5
  rw [show 2 + 2 * (3 * k + 2) + 3 * k + 2 + (5 * k + 4) = 14 * k + 12 from by ring] at h12345
  exact h12345
