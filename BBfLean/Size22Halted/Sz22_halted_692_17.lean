...
/-!
# sz22_halted_692 #17: [7/45, 125/21, 4/5, 9/2, 7/3]
...
private theorem d_drain_one_sn : ∀ a d, (⟨a + 1, 0, 0, d + 2⟩ : Q) [fm]⊢^{9} ⟨a + 12, 0, 0, d⟩ := by
  ...
  · show fm ⟨a, 1, 3, d + 1⟩ = some ⟨a, 0, 6, d⟩; simp [fm]
  apply stepNat_trans 1 5
  · show fm ⟨a, 0, 6, d⟩ = some ⟨a + 2, 0, 5, d⟩; rfl
  apply stepNat_trans 1 4
  · show fm ⟨a + 2, 0, 5, d⟩ = some ⟨a + 4, 0, 4, d⟩; rfl
  apply stepNat_trans 1 3
  · show fm ⟨a + 4, 0, 4, d⟩ = some ⟨a + 6, 0, 3, d⟩; rfl
  apply stepNat_trans 1 2
...
private theorem rem1_sn : ∀ D, (⟨0, 1, 3, D + 1⟩ : Q) [fm]⊢^{7} ⟨12, 0, 0, D⟩ := by
  intro D
  apply stepNat_trans 1 6; · show fm ⟨0, 1, 3, D + 1⟩ = some ⟨0, 0, 6, D⟩; simp [fm]
  apply stepNat_trans 1 5; · show fm ⟨0, 0, 6, D⟩ = some ⟨2, 0, 5, D⟩; rfl
  apply stepNat_trans 1 4; · show fm ⟨2, 0, 5, D⟩ = some ⟨4, 0, 4, D⟩; rfl
  apply stepNat_trans 1 3; · show fm ⟨4, 0, 4, D⟩ = some ⟨6, 0, 3, D⟩; rfl
...
private theorem rem3_sn : ∀ D, (⟨0, 3, 3, D⟩ : Q) [fm]⊢^{7} ⟨10, 0, 0, D⟩ := by
  ...
  apply stepNat_trans 1 5; · show fm ⟨0, 1, 2, D + 1⟩ = some ⟨0, 0, 5, D⟩; simp [fm]
  apply stepNat_trans 1 4; · show fm ⟨0, 0, 5, D⟩ = some ⟨2, 0, 4, D⟩; rfl
  apply stepNat_trans 1 3; · show fm ⟨2, 0, 4, D⟩ = some ⟨4, 0, 3, D⟩; rfl
...
private theorem rem5_sn : ∀ D, (⟨0, 5, 3, D⟩ : Q) [fm]⊢^{7} ⟨8, 0, 0, D + 1⟩ := by
  ...
  apply stepNat_trans 1 4; · show fm ⟨0, 1, 1, D + 2⟩ = some ⟨0, 0, 4, D + 1⟩; simp [fm]
  apply stepNat_trans 1 3; · show fm ⟨0, 0, 4, D + 1⟩ = some ⟨2, 0, 3, D + 1⟩; rfl
