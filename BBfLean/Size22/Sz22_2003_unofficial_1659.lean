...
/-!
# sz22_2003_unofficial #1659: [77/15, 4/3, 9/14, 5/11, 231/2]
...
namespace Sz22_2003_unofficial_1659
...
theorem main_trans (a n : ℕ) :
    ⟨a + n + 4, 0, n + 1, 0, 0⟩ [fm]⊢⁺ ⟨a + 2 * n + 9, 0, n + 2, 0, 0⟩ := by
  have p1 : ⟨a + n + 4, 0, n + 1, 0, 0⟩ [fm]⊢⁺ ⟨a + n + 3, 1, n + 1, 1, 1⟩ := by
    rw [show a + n + 4 = (a + n + 3) + 1 from by ring,
        show n + 1 = 0 + (n + 1) from by ring]
    step fm; finish
  have p2 : ⟨a + n + 3, 1, n + 1, 1, 1⟩ [fm]⊢* ⟨a + 2 * n + 9, 0, 0, 0, n + 2⟩ := by
    have h := interleave (2 * (n + 1) + 1) (a + 2) 0 (n + 1) 1 1 (by ring)
    rw [show (a + 2) + 2 * (n + 1) + 2 * 0 + 2 + 3 * 1 = a + 2 * n + 9 from by ring,
        show 1 + (n + 1) = n + 2 from by ring,
        show (a + 2) + (n + 1) = a + n + 3 from by ring] at h
    exact h
  have p3 : ⟨a + 2 * n + 9, 0, 0, 0, n + 2⟩ [fm]⊢* ⟨a + 2 * n + 9, 0, n + 2, 0, 0⟩ := by
    have h := r4_chain (n + 2) (a + 2 * n + 9) 0
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨a, n⟩ ↦ ⟨a + n + 4, 0, n + 1, 0, 0⟩) ⟨1, 0⟩
  intro ⟨a, n⟩
  refine ⟨⟨a + n + 4, n + 1⟩, ?_⟩
  show ⟨a + n + 4, 0, n + 1, 0, 0⟩ [fm]⊢⁺
    ⟨(a + n + 4) + (n + 1) + 4, 0, (n + 1) + 1, 0, 0⟩
  rw [show (a + n + 4) + (n + 1) + 4 = a + 2 * n + 9 from by ring]
  exact main_trans a n

end Sz22_2003_unofficial_1659
