...
/-!
# sz22_2003_unofficial #1686: [77/15, 9/14, 4/3, 5/11, 45/2]
...
namespace Sz22_2003_unofficial_1686
...
-- Odd c case: c = 2k, canonical c+1 = 2k+1
-- (a + 2k + 4, 0, 2k+1, 0, 0) ⊢⁺ (a + 4k + 9, 0, 2k+2, 0, 0)
theorem main_trans_odd (a k : ℕ) :
    ⟨a + 2 * k + 4, 0, 2 * k + 1, 0, 0⟩ [fm]⊢⁺ ⟨a + 4 * k + 9, 0, 2 * k + 2, 0, 0⟩ := by
  -- R5: (a+2k+4, 0, 2k+1, 0, 0) -> (a+2k+3, 2, 2k+2, 0, 0)
  -- R1×2: (a+2k+3, 2, 2k+2, 0, 0) -> (a+2k+3, 0, 2k, 2, 2)
  have p1 : ⟨a + 2 * k + 4, 0, 2 * k + 1, 0, 0⟩ [fm]⊢⁺
      ⟨a + 2 * k + 3, 0, 2 * k, 2, 2⟩ := by
    rw [show a + 2 * k + 4 = (a + 2 * k + 3) + 1 from by ring]
    step fm; step fm; step fm; finish
  -- R2R1R1 chain: k rounds
  -- (a+2k+3, 0, 2k, 2, 2) -> (a+k+3, 0, 0, k+2, 2k+2)
  have p2 : ⟨a + 2 * k + 3, 0, 2 * k, 2, 2⟩ [fm]⊢*
      ⟨a + k + 3, 0, 0, k + 2, 2 * k + 2⟩ := by
    have h := r2r1r1_chain k (a + k + 3) 0 1 2
    rw [show (a + k + 3) + k = a + 2 * k + 3 from by ring,
        show 0 + 2 * k = 2 * k from by ring,
        show 1 + 1 + k = k + 2 from by ring,
        show 2 + 2 * k = 2 * k + 2 from by ring] at h
    exact h
  -- R2 then drain
  -- (a+k+3, 0, 0, k+2, 2k+2) -> R2 -> (a+k+2, 2, 0, k+1, 2k+2)
  -- drain: (a+k+2, 2, 0, k+1, 2k+2) -> (a+k+2+2*1+2+3*(k+1), 0, 0, 0, 2k+2) = (a+4k+9, 0, 0, 0, 2k+2)
  have p3 : ⟨a + k + 3, 0, 0, k + 2, 2 * k + 2⟩ [fm]⊢*
      ⟨a + 4 * k + 9, 0, 0, 0, 2 * k + 2⟩ := by
    rw [show k + 2 = (k + 1) + 1 from by ring,
        show a + k + 3 = (a + k + 2) + 1 from by ring]
    step fm
    have h := drain (3 * (k + 1) + 1) (a + k + 2) 1 (k + 1) (2 * k + 2) rfl
    rw [show a + k + 2 + 2 * 1 + 2 + 3 * (k + 1) = a + 4 * k + 9 from by ring] at h
    exact h
  -- R4 chain
  have p4 : ⟨a + 4 * k + 9, 0, 0, 0, 2 * k + 2⟩ [fm]⊢*
      ⟨a + 4 * k + 9, 0, 2 * k + 2, 0, 0⟩ := by
    have h := r4_chain (2 * k + 2) (a + 4 * k + 9) 0
    simp only [Nat.zero_add] at h; exact h
  exact stepPlus_stepStar_stepPlus p1
    (stepStar_trans p2 (stepStar_trans p3 p4))

-- Even c case: c = 2k+1, canonical c+1 = 2k+2
-- (a + 2k + 5, 0, 2k+2, 0, 0) ⊢⁺ (a + 4k + 11, 0, 2k+3, 0, 0)
theorem main_trans_even (a k : ℕ) :
    ⟨a + 2 * k + 5, 0, 2 * k + 2, 0, 0⟩ [fm]⊢⁺ ⟨a + 4 * k + 11, 0, 2 * k + 3, 0, 0⟩ := by
  -- R5: (a+2k+5, 0, 2k+2, 0, 0) -> (a+2k+4, 2, 2k+3, 0, 0)
  -- R1×2: (a+2k+4, 2, 2k+3, 0, 0) -> (a+2k+4, 0, 2k+1, 2, 2)
  have p1 : ⟨a + 2 * k + 5, 0, 2 * k + 2, 0, 0⟩ [fm]⊢⁺
      ⟨a + 2 * k + 4, 0, 2 * k + 1, 2, 2⟩ := by
    rw [show a + 2 * k + 5 = (a + 2 * k + 4) + 1 from by ring]
    step fm; step fm; step fm; finish
  -- R2R1R1 chain: k rounds, remainder 1
  -- (a+2k+4, 0, 2k+1, 2, 2) -> (a+k+4, 0, 1, k+2, 2k+2)
  have p2 : ⟨a + 2 * k + 4, 0, 2 * k + 1, 2, 2⟩ [fm]⊢*
      ⟨a + k + 4, 0, 1, k + 2, 2 * k + 2⟩ := by
    have h := r2r1r1_chain k (a + k + 4) 1 1 2
  ...
  -- (a+k+4, 0, 1, k+2, 2k+2) -> R2 -> (a+k+3, 2, 1, k+1, 2k+2) -> R1 -> (a+k+3, 1, 0, k+2, 2k+3)
  have p3 : ⟨a + k + 4, 0, 1, k + 2, 2 * k + 2⟩ [fm]⊢*
      ⟨a + k + 3, 1, 0, k + 2, 2 * k + 3⟩ := by
    rw [show k + 2 = (k + 1) + 1 from by ring,
        show a + k + 4 = (a + k + 3) + 1 from by ring,
        show (1 : ℕ) = 0 + 1 from rfl]
    step fm; step fm
    rw [show 2 * k + 2 + 1 = 2 * k + 3 from by ring]; finish
  -- Drain
  have p4 : ⟨a + k + 3, 1, 0, k + 2, 2 * k + 3⟩ [fm]⊢*
      ⟨a + 4 * k + 11, 0, 0, 0, 2 * k + 3⟩ := by
    rw [show (1 : ℕ) = 0 + 1 from rfl]
    have h := drain (3 * (k + 2) + 0) (a + k + 3) 0 (k + 2) (2 * k + 3) (by ring)
    rw [show a + k + 3 + 2 * 0 + 2 + 3 * (k + 2) = a + 4 * k + 11 from by ring] at h
    exact h
  -- R4 chain
  have p5 : ⟨a + 4 * k + 11, 0, 0, 0, 2 * k + 3⟩ [fm]⊢*
      ⟨a + 4 * k + 11, 0, 2 * k + 3, 0, 0⟩ := by
    have h := r4_chain (2 * k + 3) (a + 4 * k + 11) 0
    simp only [Nat.zero_add] at h; exact h
  exact stepPlus_stepStar_stepPlus p1
    (stepStar_trans p2 (stepStar_trans p3 (stepStar_trans p4 p5)))

theorem nonhalt : ¬halts fm c₀ := by
  apply stepStar_not_halts_not_halts (c₂ := ⟨5, 0, 1, 0, 0⟩) (by execute fm 7)
  ...
  · intro q ⟨a, c, hq⟩
    ...
    · subst hk
      refine ⟨⟨a + 4 * k + 9, 0, 2 * k + 2, 0, 0⟩,
             ⟨a + 2 * k + 4, 2 * k + 1, ?_⟩, ?_⟩
      · show (a + 4 * k + 9, 0, 2 * k + 2, 0, 0) =
              ((a + 2 * k + 4) + (2 * k + 1) + 4, 0, (2 * k + 1) + 1, 0, 0)
        ring_nf
    ...
    · subst hk
      refine ⟨⟨a + 4 * k + 11, 0, 2 * k + 3, 0, 0⟩,
             ⟨a + 2 * k + 5, 2 * k + 2, ?_⟩, ?_⟩
      · show (a + 4 * k + 11, 0, 2 * k + 3, 0, 0) =
              ((a + 2 * k + 5) + (2 * k + 2) + 4, 0, (2 * k + 2) + 1, 0, 0)
        ring_nf
