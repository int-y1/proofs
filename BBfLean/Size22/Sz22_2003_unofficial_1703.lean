...
/-!
# sz22_2003_unofficial #1703: [77/15, 9/91, 26/3, 5/11, 33/2]
...
namespace Sz22_2003_unofficial_1703
...
theorem main_trans (A E : ℕ) :
    ⟨A + 1, 0, 0, 0, E + 1, E + 1⟩ [fm]⊢⁺
    ⟨A + E + 2, 0, 0, 0, E + 2, E + 2⟩ := by
  have p1 : ⟨A + 1, 0, 0, 0, E + 1, E + 1⟩ [fm]⊢*
      ⟨A + 1, 0, E + 1, 0, 0, E + 1⟩ := by
    have := e_to_c (E + 1) (A + 1) 0 (E + 1)
    simp only [Nat.zero_add] at this; exact this
  have p2 : ⟨A + 1, 0, E + 1, 0, 0, E + 1⟩ [fm]⊢⁺
      ⟨A, 1, E + 1, 0, 1, E + 1⟩ := by
    rw [show A + 1 = A + 1 from rfl]
    step fm; finish
  have p3 : ⟨A, 1, E + 1, 0, 1, E + 1⟩ [fm]⊢*
      ⟨A, E + 2, 0, 0, E + 2, 0⟩ := by
    have : ⟨A, 1, E + 1, 0, 1, 0 + (E + 1) + 0⟩ [fm]⊢*
        ⟨A, E + 2, 0, 0, E + 2, (0 : ℕ)⟩ := by
      have := combined_drain (2 * (E + 1) + 0) A 1 (E + 1) 0 1 0 (by ring)
        (by left; omega)
      rw [show 1 + (E + 1) + 2 * 0 = E + 2 from by ring,
          show 1 + (E + 1) = E + 2 from by ring] at this
      exact this
    simp only [Nat.zero_add, Nat.add_zero] at this; exact this
  have p4 : ⟨A, E + 2, 0, 0, E + 2, 0⟩ [fm]⊢*
      ⟨A + E + 2, 0, 0, 0, E + 2, E + 2⟩ := by
    have := r3_chain (E + 2) A (E + 2) 0
    rw [show A + (E + 2) = A + E + 2 from by ring,
        show 0 + (E + 2) = E + 2 from by ring] at this
    exact this
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨A, E⟩ ↦ ⟨A + 1, 0, 0, 0, E + 1, E + 1⟩) ⟨0, 0⟩
  intro ⟨A, E⟩
  refine ⟨⟨A + E + 1, E + 1⟩, ?_⟩
  show ⟨A + 1, 0, 0, 0, E + 1, E + 1⟩ [fm]⊢⁺
    ⟨A + E + 1 + 1, 0, 0, 0, E + 1 + 1, E + 1 + 1⟩
  rw [show A + E + 1 + 1 = A + E + 2 from by ring,
      show E + 1 + 1 = E + 2 from by ring]
  exact main_trans A E

end Sz22_2003_unofficial_1703
