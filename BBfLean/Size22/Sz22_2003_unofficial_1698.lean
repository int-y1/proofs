...
/-!
# sz22_2003_unofficial #1698: [77/15, 9/70, 22/3, 5/11, 21/2]
...
namespace Sz22_2003_unofficial_1698
...
theorem r2r1r1_chain : ∀ k, ∀ a c d e,
    ⟨a+k, 0, c+3*k, d+1, e⟩ [fm]⊢* ⟨a, 0, c, d+k+1, e+2*k⟩ := by
...
theorem trans_mod0 (j : ℕ) :
  ...
  step fm
  rw [show 6 * j + 2 = (6 * j + 1) + 1 from by ring]
  step fm
  apply stepStar_trans
  · rw [show 3 * j + 1 = (j + 1) + 2 * j from by ring,
        show 6 * j + 1 = 1 + 3 * (2 * j) from by ring,
        show (2 : ℕ) = 1 + 1 from rfl]
    exact r2r1r1_chain (2*j) (j+1) 1 1 1
  rw [show 1 + 2 * j + 1 = (2 * j + 1) + 1 from by ring,
      show 1 + 2 * (2 * j) = 4 * j + 1 from by ring,
  ...
  · rw [show j + 2 = (j + 1) + 1 from by ring,
        show 4 * j + 3 = (4 * j + 2) + 1 from by ring]
    exact d_drain (2*j+1) (j+1) (4*j+2)
  rw [show j + 1 + (2 * j + 1) + 1 = 3 * j + 3 from by ring,
      show 4 * j + 2 + (2 * j + 1) + 1 = 6 * j + 4 from by ring]
...
theorem trans_mod1 (j : ℕ) :
  ...
  step fm
  rw [show 6 * j + 4 = (6 * j + 3) + 1 from by ring]
  step fm
  apply stepStar_trans
  · rw [show 3 * j + 2 = (j + 1) + (2 * j + 1) from by ring,
        show 6 * j + 3 = 0 + 3 * (2 * j + 1) from by ring,
        show (2 : ℕ) = 1 + 1 from rfl]
    exact r2r1r1_chain (2*j+1) (j+1) 0 1 1
  rw [show 1 + (2 * j + 1) + 1 = 2 * j + 3 from by ring,
      show 1 + 2 * (2 * j + 1) = 4 * j + 3 from by ring]
  apply stepStar_trans
  · rw [show j + 1 = j + 1 from rfl,
        show 4 * j + 3 = (4 * j + 2) + 1 from by ring]
    exact d_drain (2*j+3) j (4*j+2)
  rw [show j + (2 * j + 3) + 1 = 3 * j + 4 from by ring,
      show 4 * j + 2 + (2 * j + 3) + 1 = 6 * j + 6 from by ring]
...
theorem trans_mod2 (j : ℕ) :
  ...
  step fm
  rw [show 6 * j + 6 = (6 * j + 5) + 1 from by ring]
  step fm
  apply stepStar_trans
  · rw [show 3 * j + 3 = (j + 2) + (2 * j + 1) from by ring,
        show 6 * j + 5 = 2 + 3 * (2 * j + 1) from by ring,
        show (2 : ℕ) = 1 + 1 from rfl]
    exact r2r1r1_chain (2*j+1) (j+2) 2 1 1
  rw [show 1 + (2 * j + 1) + 1 = 2 * j + 3 from by ring,
      show 1 + 2 * (2 * j + 1) = 4 * j + 3 from by ring]
  rw [show (2 : ℕ) = 1 + 1 from rfl,
      show 2 * j + 3 = (2 * j + 2) + 1 from by ring,
  ...
  apply stepStar_trans
  · rw [show j + 2 = (j + 1) + 1 from by ring,
        show 4 * j + 5 = (4 * j + 4) + 1 from by ring]
    exact d_drain (2*j+3) (j+1) (4*j+4)
  rw [show j + 1 + (2 * j + 3) + 1 = 3 * j + 5 from by ring,
      show 4 * j + 4 + (2 * j + 3) + 1 = 6 * j + 8 from by ring]
