...
/-!
# sz22_2003_unofficial #1669: [77/15, 8/3, 9/14, 5/11, 33/2]
...
namespace Sz22_2003_unofficial_1669
...
theorem interleave : ∀ M, ∀ A B C D E,
    3 * C + D = M → A ≥ C + D →
    ⟨A, B + 1, C, D, E⟩ [fm]⊢* ⟨A + 3 * B + 3 + 2 * C + 5 * D, 0, 0, 0, E + C⟩ := by
  intro M; induction' M using Nat.strongRecOn with M ih; intro A B C D E hM hA
  rcases C with _ | C
  · rcases D with _ | D
    · simp
      have := r2_chain (B + 1) A 0 E
      rw [show A + 3 * (B + 1) = A + 3 * B + 3 from by ring] at this
      exact this
    · have phase1 := r2_chain (B + 1) A (D + 1) E
      rw [show A + 3 * (B + 1) = A + 3 * B + 3 from by ring] at phase1
      apply stepStar_trans phase1
      rw [show A + 3 * B + 3 = (A + 3 * B + 2) + 1 from by ring, show D + 1 = D + 1 from rfl]
      step fm
      have hlt : 3 * 0 + D < M := by omega
      have hA' : A + 3 * B + 2 ≥ 0 + D := by omega
      have := ih (3 * 0 + D) hlt (A + 3 * B + 2) 1 0 D E rfl hA'
      rw [show (A + 3 * B + 2) + 3 * 1 + 3 + 2 * 0 + 5 * D =
            A + 3 * B + 3 + 2 * 0 + 5 * (D + 1) from by ring,
          show E + 0 = E + 0 from rfl] at this
      exact this
  · rcases B with _ | B
    · obtain ⟨A', rfl⟩ : ∃ A', A = A' + 1 := ⟨A - 1, by omega⟩
      show ⟨A' + 1, 0 + 1, C + 1, D, E⟩ [fm]⊢*
        ⟨A' + 1 + 3 * 0 + 3 + 2 * (C + 1) + 5 * D, 0, 0, 0, E + (C + 1)⟩
      ...
      have h := ih (3 * C + D) hlt A' 1 C D (E + 1) rfl hA'
      rw [show A' + 3 * 1 + 3 + 2 * C + 5 * D =
            A' + 1 + 3 * 0 + 3 + 2 * (C + 1) + 5 * D from by ring,
          show E + 1 + C = E + (C + 1) from by ring,
          show (1 : ℕ) + 1 = 2 from by ring] at h
      exact h
    · show ⟨A, (B + 1) + 1, C + 1, D, E⟩ [fm]⊢*
        ⟨A + 3 * (B + 1) + 3 + 2 * (C + 1) + 5 * D, 0, 0, 0, E + (C + 1)⟩
      ...
      rw [show A + 3 * B + 3 + 2 * C + 5 * (D + 1) =
            A + 3 * (B + 1) + 3 + 2 * (C + 1) + 5 * D from by ring,
          show E + 1 + C = E + (C + 1) from by ring] at h
      exact h

theorem main_trans (a n : ℕ) :
    ⟨a + n + 2, 0, n + 1, 0, 0⟩ [fm]⊢⁺ ⟨a + 3 * n + 6, 0, n + 2, 0, 0⟩ := by
  ...
  have p3 : ⟨a + n + 1, 0, n, 1, 2⟩ [fm]⊢* ⟨a + n, 2, n, 0, 2⟩ := by
    rw [show a + n + 1 = (a + n) + 1 from by ring, show (1 : ℕ) = 0 + 1 from by ring]
    step fm; ring_nf; finish
  have p4 : ⟨a + n, 2, n, 0, 2⟩ [fm]⊢* ⟨a + 3 * n + 6, 0, 0, 0, n + 2⟩ := by
    rw [show (2 : ℕ) = 1 + 1 from by ring]
    have h := interleave (3 * n + 0) (a + n) 1 n 0 2 (by ring) (by omega)
    rw [show (a + n) + 3 * 1 + 3 + 2 * n + 5 * 0 = a + 3 * n + 6 from by ring,
        show 2 + n = n + 2 from by ring] at h
    exact h
  have p5 : ⟨a + 3 * n + 6, 0, 0, 0, n + 2⟩ [fm]⊢* ⟨a + 3 * n + 6, 0, n + 2, 0, 0⟩ := by
    have h := r4_chain (n + 2) (a + 3 * n + 6) 0
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨a, n⟩ ↦ ⟨a + n + 2, 0, n + 1, 0, 0⟩) ⟨1, 0⟩
  intro ⟨a, n⟩
  refine ⟨⟨a + 2 * n + 3, n + 1⟩, ?_⟩
  show ⟨a + n + 2, 0, n + 1, 0, 0⟩ [fm]⊢⁺
    ⟨(a + 2 * n + 3) + (n + 1) + 2, 0, (n + 1) + 1, 0, 0⟩
  rw [show (a + 2 * n + 3) + (n + 1) + 2 = a + 3 * n + 6 from by ring]
  exact main_trans a n

end Sz22_2003_unofficial_1669
