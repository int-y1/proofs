...
/-!
# sz22_2003_unofficial #1045: [45/2, 4/35, 33/5, 7/99, 5/3]
...
namespace Sz22_2003_unofficial_1045
...
theorem r2r1r1_chain : ∀ k, ∀ b c d,
  ...
  · rw [show d + (k + 1) = (d + k) + 1 from by ring]
    ...
    apply stepStar_trans (ih (b + 4) (c + 1) d)
    ring_nf; finish

theorem r3_chain : ∀ k, ∀ b e,
    ⟨(0 : ℕ), b, k, 0, e⟩ [fm]⊢* ⟨0, b + k, 0, 0, e + k⟩ := by
...
theorem main_trans (b d : ℕ) :
  ...
  have h1 : ⟨(0 : ℕ), b, 1, d + 1, 0⟩ [fm]⊢* ⟨0, b + 4 * (d + 1), d + 2, 0, 0⟩ := by
    ...
    rw [show (0 : ℕ) + 1 = 1 from by ring,
        show (0 : ℕ) + (d + 1) + 1 = d + 2 from by ring,
        show (0 : ℕ) + (d + 1) = d + 1 from by ring] at this; exact this
  have h2 : ⟨(0 : ℕ), b + 4 * (d + 1), d + 2, 0, 0⟩ [fm]⊢*
      ⟨0, b + 5 * d + 6, 0, 0, d + 2⟩ := by
    have := r3_chain (d + 2) (b + 4 * (d + 1)) 0
    rw [show b + 4 * (d + 1) + (d + 2) = b + 5 * d + 6 from by ring,
        show 0 + (d + 2) = d + 2 from by ring] at this; exact this
  have h3 : ⟨(0 : ℕ), b + 5 * d + 6, 0, 0, d + 2⟩ [fm]⊢*
      ⟨0, b + 3 * d + 2, 0, d + 2, 0⟩ := by
    rw [show b + 5 * d + 6 = (b + 3 * d + 2) + 2 * (d + 2) from by ring]
    have := r4_chain (d + 2) (b + 3 * d + 2) 0
    rw [show 0 + (d + 2) = d + 2 from by ring] at this; exact this
  exact stepStar_trans h1 (stepStar_trans h2 h3)

theorem nonhalt : ¬halts fm c₀ := by
  apply stepStar_not_halts_not_halts (c₂ := ⟨0, 2, 0, 2, 0⟩) (by execute fm 11)
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨b, d⟩ ↦ ⟨0, b + 1, 0, d + 1, 0⟩) ⟨1, 1⟩
  intro ⟨b, d⟩
  exact ⟨⟨b + 3 * d + 1, d + 1⟩, main_trans b d⟩

end Sz22_2003_unofficial_1045
