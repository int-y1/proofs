...
/-!
# sz22_2003_unofficial #1705: [77/15, 9/91, 26/3, 5/11, 429/2]
...
namespace Sz22_2003_unofficial_1705
...
-- Main transition: (A+1, 0, 0, 0, E+1, 2*E+2) ⊢⁺ (A+E+2, 0, 0, 0, E+2, 2*E+4)
theorem main_trans (A E : ℕ) :
    ⟨A + 1, 0, 0, 0, E + 1, 2 * E + 2⟩ [fm]⊢⁺
    ⟨A + E + 2, 0, 0, 0, E + 2, 2 * E + 4⟩ := by
  ...
  have p2 : ⟨A + 1, 0, E + 1, 0, 0, 2 * E + 2⟩ [fm]⊢⁺
      ⟨A, 1, E + 1, 0, 1, 2 * E + 3⟩ := by
    rw [show A + 1 = A + 1 from rfl]
    step fm; finish
  -- Phase 3: R1 (b=1, c=E+1 ≥ 1) then R2 (d=1, f ≥ 1)
  have p3 : ⟨A, 1, E + 1, 0, 1, 2 * E + 3⟩ [fm]⊢*
      ⟨A, 2, E, 0, 2, 2 * E + 2⟩ := by
  ...
  -- Phase 4: Combined drain of c via interleaved R1/R2
  have p4 : ⟨A, 2, E, 0, 2, 2 * E + 2⟩ [fm]⊢*
      ⟨A, E + 2, 0, 0, E + 2, E + 2⟩ := by
    rw [show 2 * E + 2 = (E + 2) + E + 0 from by ring]
    have := combined_drain (2 * E + 0) A 2 E 0 2 (E + 2) (by ring)
      (by left; omega)
    rw [show 2 + E + 2 * 0 = E + 2 from by ring,
        show 2 + E = E + 2 from by ring] at this
    exact this
  -- Phase 5: R3 × (E+2): drain b, fill a and f
  have p5 : ⟨A, E + 2, 0, 0, E + 2, E + 2⟩ [fm]⊢*
      ⟨A + E + 2, 0, 0, 0, E + 2, 2 * E + 4⟩ := by
    have := r3_chain (E + 2) A (E + 2) (E + 2)
    rw [show A + (E + 2) = A + E + 2 from by ring,
        show E + 2 + (E + 2) = 2 * E + 4 from by ring] at this
    exact this
  -- Compose all phases
