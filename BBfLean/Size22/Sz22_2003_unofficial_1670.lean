...
/-!
# sz22_2003_unofficial #1670: [77/15, 9/14, 2/3, 5/11, 1089/2]
...
namespace Sz22_2003_unofficial_1670
...
theorem r3r2_drain : ∀ k, ∀ B E,
  ...
  | succ k ih =>
    ...
    step fm
    rw [show (1 : ℕ) = 0 + 1 from by ring, show (k + 1 : ℕ) = k + 1 from rfl]
    step fm
    apply stepStar_trans (ih (B + 1) E)
    ring_nf; finish

theorem main_trans (n : ℕ) :
    ⟨n + 1, 0, 0, 0, 2 * n⟩ [fm]⊢⁺ ⟨n + 2, 0, 0, (0 : ℕ), 2 * n + 2⟩ := by
  have p1 : ⟨n + 1, 0, 0, 0, 2 * n⟩ [fm]⊢* ⟨n + 1, 0, 2 * n, (0 : ℕ), 0⟩ := by
    have h := e_to_c (2 * n) (n + 1) 0
    rw [show 0 + 2 * n = 2 * n from by ring] at h
    exact h
  have p2 : ⟨n + 1, 0, 2 * n, 0, 0⟩ [fm]⊢⁺ ⟨n, 2, 2 * n, (0 : ℕ), 2⟩ := by
    rw [show n + 1 = n + 1 from rfl]
    step fm; finish
  have p3 : ⟨n, 2, 2 * n, 0, 2⟩ [fm]⊢* ⟨0, 2, 0, n, 2 * n + 2⟩ := by
    have h := r1r1r2_chain n 0 0 0 2
    simp only [Nat.zero_add] at h
    rw [show 2 + 2 * n = 2 * n + 2 from by ring] at h
    exact h
  have p4 : ⟨0, 2, 0, n, 2 * n + 2⟩ [fm]⊢* ⟨0, n + 2, 0, (0 : ℕ), 2 * n + 2⟩ := by
    have h := r3r2_drain n 1 (2 * n + 2)
    rw [show 1 + 1 = 2 from by ring, show 1 + 1 + n = n + 2 from by ring] at h
    exact h
  have p5 : ⟨0, n + 2, 0, 0, 2 * n + 2⟩ [fm]⊢* ⟨n + 2, 0, 0, (0 : ℕ), 2 * n + 2⟩ := by
    have h := b_to_a (n + 2) 0 (2 * n + 2)
