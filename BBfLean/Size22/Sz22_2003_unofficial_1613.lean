...
/-!
# sz22_2003_unofficial #1613: [77/15, 2/3, 27/14, 5/11, 99/2]

This Fractran program doesn't halt.

Canonical states: (A+c+2, 0, c+1, 0, 0) with quadratic growth in A.
Transition: (A+c+2, 0, c+1, 0, 0) -> (A+2c+4, 0, c+2, 0, 0), mapping (A,c) -> (A+c+1, c+1).
R1-R4 are identical to machines 1611/1612; only R5 differs (b+2, e+1).

Author: Claude Opus 4.6
-/

namespace Sz22_2003_unofficial_1613
...
-- c = 0: (A+2, 0, 1, 0, 0) -> (A+4, 0, 2, 0, 0)
theorem trans_c0 (A : ℕ) :
    ⟨A + 2, 0, 1, 0, 0⟩ [fm]⊢⁺ ⟨A + 4, 0, 2, 0, 0⟩ := by
  -- R5: (A+2, 0, 1, 0, 0) -> (A+1, 2, 1, 0, 1)
  -- R1: (A+1, 1, 0, 1, 2)
  -- R2: (A+2, 0, 0, 1, 2)
  -- r3r2_drain(1): -> (A+4, 0, 0, 0, 2)
  -- e_to_c(2): -> (A+4, 0, 2, 0, 0)
  have p1 : ⟨A + 2, 0, 1, 0, 0⟩ [fm]⊢*
      ⟨A + 2, 0, 0, 1, 2⟩ := by
    rw [show A + 2 = (A + 1) + 1 from by ring]
    step fm; step fm; step fm; ring_nf; finish
  have p2 : ⟨A + 2, 0, 0, 1, 2⟩ [fm]⊢*
      ⟨A + 4, 0, 0, 0, 2⟩ := by
    rw [show A + 2 = (A + 1) + 1 from by ring]
    have h := r3r2_drain 1 (A + 1) 2
    rw [show A + 1 + 2 * 1 + 1 = A + 4 from by ring] at h
    exact h
  have p3 : ⟨A + 4, 0, 0, 0, 2⟩ [fm]⊢*
      ⟨A + 4, 0, 2, 0, 0⟩ := by
    have h := e_to_c 2 (A + 4) 0
    simp at h; exact h
  exact stepStar_stepPlus (stepStar_trans p1 (stepStar_trans p2 p3)) (by simp [Q])

-- c ≡ 1 mod 3, c = 3j+1, j >= 0 (so c >= 1)
-- After R5+2R1: (A+c+1, 0, c-1, 2, 3) with c-1 = 3j
-- opening_rounds(j): -> (A+2j+2, 0, 0, 2j+2, 3j+3)
-- r3r2_drain(2j+2): -> (A+6j+6, 0, 0, 0, 3j+3)
-- e_to_c(3j+3): -> (A+6j+6, 0, 3j+3, 0, 0)
theorem trans_mod1 (j A : ℕ) :
    ⟨A + 3 * j + 3, 0, 3 * j + 2, 0, 0⟩ [fm]⊢⁺
    ⟨A + 6 * j + 6, 0, 3 * j + 3, 0, 0⟩ := by
  have p1 : ⟨A + 3 * j + 3, 0, 3 * j + 2, 0, 0⟩ [fm]⊢*
      ⟨A + 3 * j + 2, 0, 3 * j, 2, 3⟩ := by
    rw [show A + 3 * j + 3 = (A + 3 * j + 2) + 1 from by ring,
        show 3 * j + 2 = (3 * j + 1) + 1 from by ring,
        show 3 * j + 1 = (3 * j) + 1 from by ring]
    step fm; step fm; step fm; finish
  have p2 : ⟨A + 3 * j + 2, 0, 3 * j, 2, 3⟩ [fm]⊢*
      ⟨A + 2 * j + 2, 0, 0, 2 * j + 2, 3 * j + 3⟩ := by
    have h := opening_rounds j (A + 2 * j + 2) 0 1 3
    rw [show (A + 2 * j + 2) + j = A + 3 * j + 2 from by ring,
        show 0 + 3 * j = 3 * j from by ring,
        show 1 + 2 * j + 1 = 2 * j + 2 from by ring,
        show 3 + 3 * j = 3 * j + 3 from by ring,
        show (1 + 1 : ℕ) = 2 from by ring] at h
    exact h
  have p3 : ⟨A + 2 * j + 2, 0, 0, 2 * j + 2, 3 * j + 3⟩ [fm]⊢*
      ⟨A + 6 * j + 6, 0, 0, 0, 3 * j + 3⟩ := by
    rw [show A + 2 * j + 2 = (A + 2 * j + 1) + 1 from by ring]
    have h := r3r2_drain (2 * j + 2) (A + 2 * j + 1) (3 * j + 3)
    rw [show A + 2 * j + 1 + 2 * (2 * j + 2) + 1 = A + 6 * j + 6 from by ring] at h
    exact h
  have p4 : ⟨A + 6 * j + 6, 0, 0, 0, 3 * j + 3⟩ [fm]⊢*
      ⟨A + 6 * j + 6, 0, 3 * j + 3, 0, 0⟩ := by
    have h := e_to_c (3 * j + 3) (A + 6 * j + 6) 0
    simp at h; exact h
  exact stepStar_stepPlus (stepStar_trans p1 (stepStar_trans p2 (stepStar_trans p3 p4)))
    (by simp [Q])

-- c ≡ 2 mod 3, c = 3j+2, j >= 0 (so c >= 2)
-- After R5+2R1: (A+c+1, 0, c-1, 2, 3) with c-1 = 3j+1
-- opening_rounds(j): -> (A+2j+3, 0, 1, 2j+2, 3j+3)
-- tail_c1: -> (A+2j+4, 0, 0, 2j+2, 3j+4)
-- r3r2_drain(2j+2): -> (A+6j+8, 0, 0, 0, 3j+4)
-- e_to_c(3j+4): -> (A+6j+8, 0, 3j+4, 0, 0)
theorem trans_mod2 (j A : ℕ) :
    ⟨A + 3 * j + 4, 0, 3 * j + 3, 0, 0⟩ [fm]⊢⁺
    ⟨A + 6 * j + 8, 0, 3 * j + 4, 0, 0⟩ := by
  have p1 : ⟨A + 3 * j + 4, 0, 3 * j + 3, 0, 0⟩ [fm]⊢*
      ⟨A + 3 * j + 3, 0, 3 * j + 1, 2, 3⟩ := by
    rw [show A + 3 * j + 4 = (A + 3 * j + 3) + 1 from by ring,
        show 3 * j + 3 = (3 * j + 2) + 1 from by ring,
        show 3 * j + 2 = (3 * j + 1) + 1 from by ring]
    step fm; step fm; step fm; finish
  have p2 : ⟨A + 3 * j + 3, 0, 3 * j + 1, 2, 3⟩ [fm]⊢*
      ⟨A + 2 * j + 3, 0, 1, 2 * j + 2, 3 * j + 3⟩ := by
    have h := opening_rounds j (A + 2 * j + 3) 1 1 3
    rw [show (A + 2 * j + 3) + j = A + 3 * j + 3 from by ring,
        ...
        show (1 + 1 : ℕ) = 2 from by ring] at h
    exact h
  have p3 : ⟨A + 2 * j + 3, 0, 1, 2 * j + 2, 3 * j + 3⟩ [fm]⊢*
      ⟨A + 2 * j + 4, 0, 0, 2 * j + 2, 3 * j + 4⟩ := by
    rw [show A + 2 * j + 3 = (A + 2 * j + 2) + 1 from by ring,
        show 2 * j + 2 = (2 * j + 1) + 1 from by ring]
    have h := tail_c1 (A + 2 * j + 2) (2 * j + 1) (3 * j + 3)
    rw [show A + 2 * j + 2 + 2 = A + 2 * j + 4 from by ring,
        show 2 * j + 1 + 1 = 2 * j + 2 from by ring] at h
    exact h
  have p4 : ⟨A + 2 * j + 4, 0, 0, 2 * j + 2, 3 * j + 4⟩ [fm]⊢*
      ⟨A + 6 * j + 8, 0, 0, 0, 3 * j + 4⟩ := by
    rw [show A + 2 * j + 4 = (A + 2 * j + 3) + 1 from by ring]
    have h := r3r2_drain (2 * j + 2) (A + 2 * j + 3) (3 * j + 4)
    rw [show A + 2 * j + 3 + 2 * (2 * j + 2) + 1 = A + 6 * j + 8 from by ring] at h
    exact h
  have p5 : ⟨A + 6 * j + 8, 0, 0, 0, 3 * j + 4⟩ [fm]⊢*
      ⟨A + 6 * j + 8, 0, 3 * j + 4, 0, 0⟩ := by
    have h := e_to_c (3 * j + 4) (A + 6 * j + 8) 0
    simp at h; exact h
  exact stepStar_stepPlus (stepStar_trans p1 (stepStar_trans p2 (stepStar_trans p3
    (stepStar_trans p4 p5)))) (by simp [Q])

-- c ≡ 0 mod 3, c = 3(j+1), j >= 0 (so c >= 3)
-- After R5+2R1: (A+c+1, 0, c-1, 2, 3) with c-1 = 3j+2
-- opening_rounds(j): -> (A+2j+4, 0, 2, 2j+2, 3j+3)
-- tail_c2: -> (A+2j+4, 0, 0, 2j+3, 3j+5)
-- r3r2_drain(2j+3): -> (A+6j+10, 0, 0, 0, 3j+5)
-- e_to_c(3j+5): -> (A+6j+10, 0, 3j+5, 0, 0)
theorem trans_mod0 (j A : ℕ) :
    ⟨A + 3 * j + 5, 0, 3 * j + 4, 0, 0⟩ [fm]⊢⁺
    ⟨A + 6 * j + 10, 0, 3 * j + 5, 0, 0⟩ := by
  have p1 : ⟨A + 3 * j + 5, 0, 3 * j + 4, 0, 0⟩ [fm]⊢*
      ⟨A + 3 * j + 4, 0, 3 * j + 2, 2, 3⟩ := by
    rw [show A + 3 * j + 5 = (A + 3 * j + 4) + 1 from by ring,
        show 3 * j + 4 = (3 * j + 3) + 1 from by ring,
        show 3 * j + 3 = (3 * j + 2) + 1 from by ring]
    step fm; step fm; step fm; finish
  have p2 : ⟨A + 3 * j + 4, 0, 3 * j + 2, 2, 3⟩ [fm]⊢*
      ⟨A + 2 * j + 4, 0, 2, 2 * j + 2, 3 * j + 3⟩ := by
    have h := opening_rounds j (A + 2 * j + 4) 2 1 3
    rw [show (A + 2 * j + 4) + j = A + 3 * j + 4 from by ring,
        ...
        show (1 + 1 : ℕ) = 2 from by ring] at h
    exact h
  have p3 : ⟨A + 2 * j + 4, 0, 2, 2 * j + 2, 3 * j + 3⟩ [fm]⊢*
      ⟨A + 2 * j + 4, 0, 0, 2 * j + 3, 3 * j + 5⟩ := by
    rw [show A + 2 * j + 4 = (A + 2 * j + 3) + 1 from by ring,
        show 2 * j + 2 = (2 * j + 1) + 1 from by ring]
    have h := tail_c2 (A + 2 * j + 3) (2 * j + 1) (3 * j + 3)
    rw [show A + 2 * j + 3 + 1 = A + 2 * j + 4 from by ring,
        show 2 * j + 1 + 2 = 2 * j + 3 from by ring] at h
    exact h
  have p4 : ⟨A + 2 * j + 4, 0, 0, 2 * j + 3, 3 * j + 5⟩ [fm]⊢*
      ⟨A + 6 * j + 10, 0, 0, 0, 3 * j + 5⟩ := by
    rw [show A + 2 * j + 4 = (A + 2 * j + 3) + 1 from by ring]
    have h := r3r2_drain (2 * j + 3) (A + 2 * j + 3) (3 * j + 5)
    rw [show A + 2 * j + 3 + 2 * (2 * j + 3) + 1 = A + 6 * j + 10 from by ring] at h
    exact h
  have p5 : ⟨A + 6 * j + 10, 0, 0, 0, 3 * j + 5⟩ [fm]⊢*
      ⟨A + 6 * j + 10, 0, 3 * j + 5, 0, 0⟩ := by
    have h := e_to_c (3 * j + 5) (A + 6 * j + 10) 0
    simp at h; exact h
  exact stepStar_stepPlus (stepStar_trans p1 (stepStar_trans p2 (stepStar_trans p3
    (stepStar_trans p4 p5)))) (by simp [Q])
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  apply progress_nonhalt (fm := fm)
    (P := fun q ↦ ∃ A c, q = ⟨A + c + 2, 0, c + 1, 0, 0⟩)
  · rintro q ⟨A, c, rfl⟩
    rcases c with _ | c
    · -- c = 0: state is (A+2, 0, 1, 0, 0)
      refine ⟨⟨A + 4, 0, 2, 0, 0⟩, ⟨A + 1, 1, by ring_nf⟩, ?_⟩
      exact trans_c0 A
    · -- c >= 1: state is (A+c+3, 0, c+2, 0, 0)
      ...
      · obtain ⟨j, rfl⟩ := Nat.dvd_of_mod_eq_zero (by omega : c % 3 = 0)
        refine ⟨⟨A + 6 * j + 6, 0, 3 * j + 3, 0, 0⟩,
                ⟨A + 3 * j + 2, 3 * j + 2, by ring_nf⟩, ?_⟩
        exact trans_mod1 j A
      · have ⟨j, hj⟩ : ∃ j, c = 3 * j + 1 := ⟨c / 3, by omega⟩
        subst hj
        refine ⟨⟨A + 6 * j + 8, 0, 3 * j + 4, 0, 0⟩,
                ⟨A + 3 * j + 3, 3 * j + 3, by ring_nf⟩, ?_⟩
        exact trans_mod2 j A
      · have ⟨j, hj⟩ : ∃ j, c = 3 * j + 2 := ⟨c / 3, by omega⟩
        subst hj
        refine ⟨⟨A + 6 * j + 10, 0, 3 * j + 5, 0, 0⟩,
                ⟨A + 3 * j + 4, 3 * j + 4, by ring_nf⟩, ?_⟩
        exact trans_mod0 j A
  · exact ⟨0, 0, by simp⟩

end Sz22_2003_unofficial_1613
