...
/-!
# sz22_2003_unofficial #1729: [8/15, 33/14, 25/2, 7/11, 33/5]
...
namespace Sz22_2003_unofficial_1729
...
theorem main_trans (n C : ℕ) :
    ⟨0, 0, C + n + 3, 0, n + 1⟩ [fm]⊢⁺ ⟨0, 0, C + 4 * n + 10, 0, n + 2⟩ := by
  have p1 : ⟨0, 0, C + n + 3, 0, n + 1⟩ [fm]⊢*
      ⟨0, 0, C + n + 3, n + 1, 0⟩ := by
    have h := r4_chain (n + 1) (C + n + 3) 0
    simp at h; exact h
  have p2 : ⟨0, 0, C + n + 3, n + 1, 0⟩ [fm]⊢⁺
      ⟨3, 0, C + n + 1, n + 1, 1⟩ := by
    rw [show C + n + 3 = (C + n + 2) + 1 from by ring]
    step fm
    rw [show C + n + 2 = (C + n + 1) + 1 from by ring]
    step fm; finish
  have p3 : ⟨3, 0, C + n + 1, n + 1, 1⟩ [fm]⊢*
      ⟨2 * n + 5, 0, C, 0, n + 2⟩ := by
    have h := r2r1_chain (n + 1) 2 C 0 1
    rw [show 2 + 2 * (n + 1) + 1 = 2 * n + 5 from by ring,
  ...
  have p4 : ⟨2 * n + 5, 0, C, 0, n + 2⟩ [fm]⊢*
      ⟨0, 0, C + 2 * (2 * n + 5), 0, n + 2⟩ := by
    have h := r3_chain (2 * n + 5) 0 C (n + 2)
    simp at h; exact h
  have goal_rw : C + 2 * (2 * n + 5) = C + 4 * n + 10 := by ring
  rw [goal_rw] at p4
...
theorem nonhalt : ¬halts fm c₀ := by
  apply stepStar_not_halts_not_halts (c₂ := ⟨0, 0, 6, 0, 1⟩) (by execute fm 6)
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨n, C⟩ ↦ ⟨0, 0, C + n + 3, 0, n + 1⟩) ⟨0, 3⟩
  intro ⟨n, C⟩
  refine ⟨⟨n + 1, C + 3 * n + 6⟩, ?_⟩
  show ⟨0, 0, C + n + 3, 0, n + 1⟩ [fm]⊢⁺ ⟨0, 0, (C + 3 * n + 6) + (n + 1) + 3, 0, (n + 1) + 1⟩
  rw [show (C + 3 * n + 6) + (n + 1) + 3 = C + 4 * n + 10 from by ring]
  exact main_trans n C

end Sz22_2003_unofficial_1729
