...
/-!
# sz22_2003_unofficial #1663: [77/15, 4/3, 9/14, 5/11, 99/2]
...
namespace Sz22_2003_unofficial_1663
...
theorem main_trans (a n : ℕ) :
    ⟨a + n + 4, 0, n + 1, 0, 0⟩ [fm]⊢⁺ ⟨a + 2 * n + 8, 0, n + 2, 0, 0⟩ := by
  have p1 : ⟨a + n + 4, 0, n + 1, 0, 0⟩ [fm]⊢⁺
      ⟨a + n + 3, 1, n, 1, 2⟩ := by
    ...
    step fm; finish
  have p2 : ⟨a + n + 3, 1, n, 1, 2⟩ [fm]⊢*
      ⟨a + 2 * n + 8, 0, 0, 0, n + 2⟩ := by
    have h := interleave (2 * n + 1) (a + 3) 0 n 1 2 (by ring)
    rw [show (a + 3) + 2 * n + 2 * 0 + 2 + 3 * 1 = a + 2 * n + 8 from by ring,
        show 2 + n = n + 2 from by ring,
        show (a + 3) + n = a + n + 3 from by ring] at h
    exact h
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨a, n⟩ ↦ ⟨a + n + 4, 0, n + 1, 0, 0⟩) ⟨0, 0⟩
  intro ⟨a, n⟩
  refine ⟨⟨a + n + 3, n + 1⟩, ?_⟩
  show ⟨a + n + 4, 0, n + 1, 0, 0⟩ [fm]⊢⁺
    ⟨(a + n + 3) + (n + 1) + 4, 0, (n + 1) + 1, 0, 0⟩
  rw [show (a + n + 3) + (n + 1) + 4 = a + 2 * n + 8 from by ring]
  exact main_trans a n

end Sz22_2003_unofficial_1663
