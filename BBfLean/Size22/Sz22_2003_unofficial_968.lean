...
/-!
# sz22_2003_unofficial #968: [4/15, 33/14, 5/2, 7/11, 484/5]
...
namespace Sz22_2003_unofficial_968
...
theorem main_trans (n : ℕ) :
  ...
  have h1 : ⟨0, 0, n + 1, 0, 2 * n⟩ [fm]⊢* ⟨0, 0, n + 1, 2 * n, 0⟩ := by
    have := r4_drain (2 * n) (n + 1) 0
    rw [show 0 + 2 * n = 2 * n from by ring] at this
    exact this
  have h2 : ⟨0, 0, n + 1, 2 * n, 0⟩ [fm]⊢⁺ ⟨2, 0, n, 2 * n, 2⟩ := by
    apply step_stepPlus
    show fm ⟨0, 0, n + 1, 2 * n, 0⟩ = some ⟨2, 0, n, 2 * n, 2⟩
    unfold fm; simp only
  have h3 : ⟨2, 0, n, 2 * n, 2⟩ [fm]⊢* ⟨n + 2, 0, 0, n, n + 2⟩ := by
    have := r2r1_chain n 0 0 n 2
    rw [show 0 + 2 = 2 from by ring,
        show 0 + n = n from by ring,
        show n + n = 2 * n from by ring,
        show 0 + 2 + n = n + 2 from by ring] at this
    exact this
  have h4 : ⟨n + 2, 0, 0, n, n + 2⟩ [fm]⊢* ⟨2, n, 0, 0, 2 * n + 2⟩ := by
    have := r2_drain n 2 0 (n + 2)
    rw [show 2 + n = n + 2 from by ring,
        show 0 + n = n from by ring,
        show (n + 2) + n = 2 * n + 2 from by ring] at this
    exact this
  have h5 : ⟨2, n, 0, 0, 2 * n + 2⟩ [fm]⊢* ⟨n + 2, 0, 0, 0, 2 * n + 2⟩ := by
    have := r3r1_chain n 1 0 (2 * n + 2)
    rw [show 1 + 1 = 2 from by ring,
        show 0 + n = n from by ring,
        show 1 + 1 + n = n + 2 from by ring] at this
    exact this
  have h6 : ⟨n + 2, 0, 0, 0, 2 * n + 2⟩ [fm]⊢* ⟨0, 0, n + 2, 0, 2 * n + 2⟩ := by
    have := r3_drain (n + 2) 0 (2 * n + 2)
