...
/-!
# sz22_2003_unofficial #1706: [77/15, 9/91, 26/3, 5/11, 45/2]
...
namespace Sz22_2003_unofficial_1706
...
-- Main transition: (A+1, 0, 0, 0, E+1, 2*E+2) ⊢⁺ (A+E+4, 0, 0, 0, E+2, 2*E+4)
theorem main_trans (A E : ℕ) :
  ...
  have p2 : ⟨A + 1, 0, E + 1, 0, 0, 2 * E + 2⟩ [fm]⊢⁺
      ⟨A, 2, E + 2, 0, 0, 2 * E + 2⟩ := by
    rw [show A + 1 = A + 1 from rfl]
    step fm; finish
  -- Phase 3: Combined drain of c via interleaved R1/R2
  have p3 : ⟨A, 2, E + 2, 0, 0, 2 * E + 2⟩ [fm]⊢*
      ⟨A, E + 4, 0, 0, E + 2, E⟩ := by
    rw [show 2 * E + 2 = E + (E + 2) + 0 from by ring]
    have := combined_drain (2 * (E + 2) + 0) A 2 (E + 2) 0 0 E (by ring)
      (by left; omega)
    rw [show 2 + (E + 2) + 2 * 0 = E + 4 from by ring,
        show 0 + (E + 2) = E + 2 from by ring] at this
    exact this
  -- Phase 4: R3 × (E+4): drain b, fill a and f
  have p4 : ⟨A, E + 4, 0, 0, E + 2, E⟩ [fm]⊢*
      ⟨A + E + 4, 0, 0, 0, E + 2, 2 * E + 4⟩ := by
...
theorem nonhalt : ¬halts fm c₀ := by
  apply stepStar_not_halts_not_halts (c₂ := ⟨3, 0, 0, 0, 1, 2⟩) (by execute fm 6)
