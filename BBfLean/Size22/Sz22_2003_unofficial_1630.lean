...
/-!
# sz22_2003_unofficial #1630: [77/15, 26/3, 9/91, 5/11, 195/2]
...
namespace Sz22_2003_unofficial_1630
...
theorem r3r1r1_chain : ∀ k, ∀ α c d e f,
    ⟨α, 0, c + 2 * k, d + 1, e, f + k + 1⟩ [fm]⊢*
    ⟨α, 0, c, d + k + 1, e + 2 * k, f + 1⟩ := by
...
theorem main_trans_odd (m A : ℕ) :
    ⟨A + 2, 0, 0, 0, 2 * m + 1, 4 * m + 2⟩ [fm]⊢⁺
    ⟨A + 2 * m + 4, 0, 0, 0, 2 * m + 2, 4 * m + 4⟩ := by
  have p1 : ⟨A + 2, 0, 0, 0, 2 * m + 1, 4 * m + 2⟩ [fm]⊢*
      ⟨A + 2, 0, 2 * m + 1, 0, 0, 4 * m + 2⟩ := by
    have := e_to_c (2 * m + 1) (A + 2) 0 (4 * m + 2)
    simp only [Nat.zero_add] at this; exact this
  have p2 : ⟨A + 2, 0, 2 * m + 1, 0, 0, 4 * m + 2⟩ [fm]⊢⁺
      ⟨A + 1, 0, 2 * m + 1, 1, 1, 4 * m + 3⟩ := by
    rw [show A + 2 = (A + 1) + 1 from by ring,
        show 2 * m + 1 = (2 * m) + 1 from by ring,
        show 4 * m + 2 = (4 * m + 1) + 1 from by ring]
    step fm
    rw [show 2 * m + 1 + 1 = (2 * m + 1) + 1 from by ring,
        show 4 * m + 1 + 1 + 1 = 4 * m + 3 from by ring]
    step fm; finish
  have p3 : ⟨A + 1, 0, 2 * m + 1, 1, 1, 4 * m + 3⟩ [fm]⊢*
      ⟨A + 1, 0, 1, m + 1, 2 * m + 1, 3 * m + 3⟩ := by
    have h := r3r1r1_chain m (A + 1) 1 0 1 (3 * m + 2)
    ...
    simpa using h
  have p3b : ⟨A + 1, 0, 1, m + 1, 2 * m + 1, 3 * m + 3⟩ [fm]⊢*
      ⟨A + 2, 0, 0, m + 1, 2 * m + 2, 3 * m + 3⟩ := by
    rw [show 3 * m + 3 = (3 * m + 2) + 1 from by ring]
    step fm; step fm; step fm; ring_nf; finish
  have p4 : ⟨A + 2, 0, 0, m + 1, 2 * m + 2, 3 * m + 3⟩ [fm]⊢*
      ⟨A + 2 * m + 4, 0, 0, 0, 2 * m + 2, 4 * m + 4⟩ := by
...
theorem main_trans_even (m A : ℕ) :
    ⟨A + 2, 0, 0, 0, 2 * m + 2, 4 * m + 4⟩ [fm]⊢⁺
    ⟨A + 2 * m + 5, 0, 0, 0, 2 * m + 3, 4 * m + 6⟩ := by
  have p1 : ⟨A + 2, 0, 0, 0, 2 * m + 2, 4 * m + 4⟩ [fm]⊢*
      ⟨A + 2, 0, 2 * m + 2, 0, 0, 4 * m + 4⟩ := by
    have := e_to_c (2 * m + 2) (A + 2) 0 (4 * m + 4)
    simp only [Nat.zero_add] at this; exact this
  have p2 : ⟨A + 2, 0, 2 * m + 2, 0, 0, 4 * m + 4⟩ [fm]⊢⁺
      ⟨A + 1, 0, 2 * m + 2, 1, 1, 4 * m + 5⟩ := by
    rw [show A + 2 = (A + 1) + 1 from by ring,
        show 2 * m + 2 = (2 * m + 1) + 1 from by ring,
        show 4 * m + 4 = (4 * m + 3) + 1 from by ring]
    step fm
    rw [show 2 * m + 2 + 1 = (2 * m + 2) + 1 from by ring,
        show 4 * m + 3 + 1 + 1 = 4 * m + 5 from by ring]
    step fm; finish
  have p3 : ⟨A + 1, 0, 2 * m + 2, 1, 1, 4 * m + 5⟩ [fm]⊢*
      ⟨A + 1, 0, 0, m + 2, 2 * m + 3, 3 * m + 4⟩ := by
    have h := r3r1r1_chain (m + 1) (A + 1) 0 0 1 (3 * m + 3)
    rw [show 0 + 2 * (m + 1) = 2 * m + 2 from by ring,
        show 3 * m + 3 + (m + 1) + 1 = 4 * m + 5 from by ring,
        show 0 + (m + 1) + 1 = m + 2 from by ring,
        show 1 + 2 * (m + 1) = 2 * m + 3 from by ring,
        show 3 * m + 3 + 1 = 3 * m + 4 from by ring] at h
    simpa using h
