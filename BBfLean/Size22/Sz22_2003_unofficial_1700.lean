...
/-!
# sz22_2003_unofficial #1700: [77/15, 9/91, 26/3, 5/11, 1001/2]
...
namespace Sz22_2003_unofficial_1700
...
-- R1 fires C times total, R2 fires C+D times total.
-- Result: b += C+2D, e += C, f -= C+D.
theorem combined_drain : ∀ M, ∀ X B C D E F, 2 * C + D = M →
  ...
  · -- C ≥ 1
    ...
    · -- C ≥ 1, D ≥ 1
      rcases B with _ | B
      · -- B = 0: R2 fires (b=0 so R1 can't fire)
        rw [show D + 1 = D + 1 from rfl,
        ...
        exact this
      · -- B ≥ 1, C ≥ 1: R1 fires (higher priority than R2)
        rw [show B + 1 = B + 1 from rfl, show C + 1 = C + 1 from rfl]
        step fm
...
-- Main transition: (A+1, 0, 0, 0, E+1, 2*E+2) ⊢⁺ (A+E+3, 0, 0, 0, E+2, 2*E+4)
theorem main_trans (A E : ℕ) :
  ...
  have p4 : ⟨A, 2, E + 1, 0, 1, 2 * E + 2⟩ [fm]⊢*
    ...
    exact this
  -- Phase 5: R3 × (E+3): drain b, fill a and f
...
theorem nonhalt : ¬halts fm c₀ := by
  apply stepStar_not_halts_not_halts (c₂ := ⟨2, 0, 0, 0, 1, 2⟩) (by execute fm 4)
