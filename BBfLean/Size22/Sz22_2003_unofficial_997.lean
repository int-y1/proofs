...
/-!
# sz22_2003_unofficial #997: [4/15, 33/14, 845/2, 7/11, 22/13]
...
namespace Sz22_2003_unofficial_997
...
theorem main_trans (n f : ℕ) :
    ⟨0, 0, n + 1, 0, n, f + 1⟩ [fm]⊢⁺ ⟨0, 0, n + 2, 0, n + 1, f + 2 * n + 2⟩ := by
  have h1 : ⟨0, 0, n + 1, 0, n, f + 1⟩ [fm]⊢* ⟨0, 0, n + 1, n, 0, f + 1⟩ := by
    have := r4_chain n (n + 1) 0 (f + 1)
    rw [show 0 + n = n from by ring] at this; exact this
  have h2 : ⟨0, 0, n + 1, n, 0, f + 1⟩ [fm]⊢⁺ ⟨1, 0, n + 1, n, 1, f⟩ := by
    rw [show f + 1 = f + 1 from rfl]
    step fm; finish
  have h3 : ⟨1, 0, n + 1, n, 1, f⟩ [fm]⊢* ⟨n + 1, 0, 1, 0, n + 1, f⟩ := by
    have := r2r1_chain n 0 1 0 1 f
    rw [show 0 + 1 = 1 from by ring, show 1 + n = n + 1 from by ring,
        show 0 + n = n from by ring] at this
    exact this
  have h4 : ⟨n + 1, 0, 1, 0, n + 1, f⟩ [fm]⊢* ⟨0, 0, n + 2, 0, n + 1, f + 2 * n + 2⟩ := by
    have := r3_chain (n + 1) 1 (n + 1) f
    rw [show 1 + (n + 1) = n + 2 from by ring,
        show f + 2 * (n + 1) = f + 2 * n + 2 from by ring] at this
    exact this
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  apply progress_nonhalt_simple (fm := fm) (A := ℕ × ℕ)
    (fun ⟨n, f⟩ ↦ ⟨0, 0, n + 1, 0, n, f + 1⟩) ⟨0, 1⟩
  intro ⟨n, f⟩
  refine ⟨⟨n + 1, f + 2 * n + 1⟩, ?_⟩
  show ⟨0, 0, n + 1, 0, n, f + 1⟩ [fm]⊢⁺ ⟨0, 0, n + 2, 0, n + 1, f + 2 * n + 1 + 1⟩
  rw [show f + 2 * n + 1 + 1 = f + 2 * n + 2 from by ring]
