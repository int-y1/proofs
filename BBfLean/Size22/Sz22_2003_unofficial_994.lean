...
/-!
# sz22_2003_unofficial #994: [4/15, 33/14, 65/2, 91/11, 21/13]
...
namespace Sz22_2003_unofficial_994
...
theorem r4_chain : ∀ k, ∀ c d f, ⟨0, 0, c, d, k, f⟩ [fm]⊢* ⟨0, 0, c, d + k, 0, f + k⟩ := by
...
theorem main_trans (n f : ℕ) :
    ⟨0, 0, n + 1, 0, n, f + 1⟩ [fm]⊢⁺ ⟨0, 0, n + 2, 0, n + 1, f + 1 + 2 * (n + 1)⟩ := by
  have h1 : ⟨0, 0, n + 1, 0, n, f + 1⟩ [fm]⊢* ⟨0, 0, n + 1, n, 0, f + 1 + n⟩ := by
    have := r4_chain n (n + 1) 0 (f + 1)
    rw [show 0 + n = n from by ring] at this; exact this
  have h2 : ⟨0, 0, n + 1, n, 0, f + 1 + n⟩ [fm]⊢⁺ ⟨0, 1, n + 1, n + 1, 0, f + n⟩ := by
    rw [show f + 1 + n = (f + n) + 1 from by ring]
    step fm; finish
  have h3 : ⟨0, 1, n + 1, n + 1, 0, f + n⟩ [fm]⊢* ⟨2, 0, n, n + 1, 0, f + n⟩ := by
    rw [show n + 1 = n + 1 from rfl]
    step fm; finish
  have h4 : ⟨2, 0, n, n + 1, 0, f + n⟩ [fm]⊢* ⟨1, 1, n, n, 1, f + n⟩ := by
    rw [show n + 1 = n + 1 from rfl, show (2 : ℕ) = 1 + 1 from by ring]
    step fm; ring_nf; finish
  have h5 : ⟨1, 1, n, n, 1, f + n⟩ [fm]⊢* ⟨n + 1, 1, 0, 0, n + 1, f + n⟩ := by
    have := r1r2_chain n 1 0 0 1 (f + n)
    simp only [Nat.zero_add, Nat.add_comm 1 n] at this
    exact this
  have h6 : ⟨n + 1, 1, 0, 0, n + 1, f + n⟩ [fm]⊢* ⟨n, 1, 1, 0, n + 1, f + n + 1⟩ := by
    step fm; ring_nf; finish
  have h7 : ⟨n, 1, 1, 0, n + 1, f + n + 1⟩ [fm]⊢* ⟨n + 2, 0, 0, 0, n + 1, f + n + 1⟩ := by
    step fm; ring_nf; finish
  have h8 : ⟨n + 2, 0, 0, 0, n + 1, f + n + 1⟩ [fm]⊢*
      ⟨0, 0, n + 2, 0, n + 1, f + n + 1 + (n + 2)⟩ := by
    have := r3_chain (n + 2) 0 (n + 1) (f + n + 1)
    rw [show 0 + (n + 2) = n + 2 from by ring] at this; exact this
  have heq : f + n + 1 + (n + 2) = f + 1 + 2 * (n + 1) := by ring
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  show ⟨0, 0, n + 1, 0, n, f + 1⟩ [fm]⊢⁺ ⟨0, 0, n + 1 + 1, 0, n + 1, f + 2 * (n + 1) + 1⟩
