...
/-!
# sz22_2003_unofficial #181: [1/6, 1225/33, 3/5, 4/7, 605/2]
...
namespace Sz22_2003_unofficial_181
...
-- Phase 1: R5,R3,R1 chain: (a+2, 0, 0, 0, e) ->* (a, 0, 0, 0, e+2) in 3 steps
theorem r5r3r1_chain : ∀ j a e,
...
-- Main transition: (2k+1, 0, 0, 0, 0) ->+ (2*(3k+2)+1, 0, 0, 0, 0)
theorem main_trans (k : ℕ) :
    ⟨2*k+1, 0, 0, 0, 0⟩ [fm]⊢⁺ ⟨2*(3*k+2)+1, 0, 0, 0, 0⟩ := by
  -- Phase 1: R5,R3,R1 chain then final R5,R3
  -- (2k+1, 0, 0, 0, 0) ->* (1, 0, 0, 0, 2k) -> (0, 0, 1, 0, 2k+2) -> (0, 1, 0, 0, 2k+2)
  apply stepStar_stepPlus_stepPlus
  · have h := r5r3r1_chain k 1 0
    rw [show 1 + 2 * k = 2 * k + 1 from by ring,
        show 0 + 2 * k = 2 * k from by ring] at h
    exact h
  -- Now at (1, 0, 0, 0, 2k). R5 fires, then R3.
  apply step_stepStar_stepPlus
  · show fm ⟨1, 0, 0, 0, 2 * k⟩ = some ⟨0, 0, 1, 0, 2 * k + 2⟩; rfl
  -- R3: (0, 0, 1, 0, 2k+2) -> (0, 1, 0, 0, 2k+2)
  apply stepStar_trans
  · show ⟨0, 0, 1, 0, 2 * k + 2⟩ [fm]⊢* ⟨0, 1, 0, 0, 2 * k + 2⟩
    step fm; finish
