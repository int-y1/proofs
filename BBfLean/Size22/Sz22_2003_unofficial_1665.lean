...
/-!
# sz22_2003_unofficial #1665: [77/15, 4/3, 99/14, 5/11, 33/2]
...
namespace Sz22_2003_unofficial_1665
...
theorem main_trans (n : ℕ) :
    ⟨n + 2, 0, 0, 0, n + 1⟩ [fm]⊢⁺
    ⟨2 * n + 4, 0, 0, 0, 2 * n + 3⟩ := by
  have p1 : ⟨n + 2, 0, 0, 0, n + 1⟩ [fm]⊢*
            ⟨n + 2, 0, n + 1, 0, 0⟩ := by
    have h := r4_chain (n + 1) (n + 2) 0; simp at h; exact h
  have p2 : ⟨n + 2, 0, n + 1, 0, 0⟩ [fm]⊢⁺
            ⟨n + 1, 1, n + 1, 0, 1⟩ := by
    rw [show n + 2 = (n + 1) + 1 from by ring]
    step fm; finish
  have p3 : ⟨n + 1, 1, n + 1, 0, 1⟩ [fm]⊢*
            ⟨n + 1, 0, n, 1, 2⟩ := by
    rw [show (1 : ℕ) = 0 + 1 from rfl,
        show n + 1 = n + 1 from rfl]
    step fm; ring_nf; finish
  rcases Nat.even_or_odd n with ⟨K, hK⟩ | ⟨K, hK⟩
  · have p4 : ⟨n + 1, 0, n, 1, 2⟩ [fm]⊢*
              ⟨K + 1, 0, 0, K + 1, 2 + 3 * K⟩ := by
      rw [show n + 1 = K + K + 1 from by omega,
          show n = 2 * K from by omega,
          show (1 : ℕ) = 0 + 1 from rfl]
      have h := interleave_even K K 0 2
      apply stepStar_trans h
      rw [show 0 + K + 1 = K + 1 from by ring]
      finish
    have p5 : ⟨K + 1, 0, 0, K + 1, 2 + 3 * K⟩ [fm]⊢*
              ⟨4 * K + 4, 0, 0, 0, 4 * K + 3⟩ := by
      rw [show K + 1 = K + 1 from rfl]
      have h := r3r2r2_chain (K + 1) K (2 + 3 * K)
      apply stepStar_trans h
      rw [show K + 3 * (K + 1) + 1 = 4 * K + 4 from by ring,
          show (2 + 3 * K) + (K + 1) = 4 * K + 3 from by ring]
      finish
    rw [show 2 * n + 4 = 4 * K + 4 from by omega,
        show 2 * n + 3 = 4 * K + 3 from by omega]
    exact stepStar_stepPlus_stepPlus p1
      (stepPlus_stepStar_stepPlus p2 (stepStar_trans p3 (stepStar_trans p4 p5)))
  · have p4 : ⟨n + 1, 0, n, 1, 2⟩ [fm]⊢*
              ⟨K + 3, 0, 0, K + 1, 3 * K + 4⟩ := by
      rw [show n + 1 = (K + 1) + K + 1 from by omega,
          show n = 2 * K + 1 from by omega,
          show (1 : ℕ) = 0 + 1 from rfl]
      have h := interleave_odd K (K + 1) 0 2
      apply stepStar_trans h
      rw [show (K + 1) + 2 = K + 3 from by ring,
          show 0 + K + 1 = K + 1 from by ring,
          show 2 + 3 * K + 2 = 3 * K + 4 from by ring]
      finish
    have p5 : ⟨K + 3, 0, 0, K + 1, 3 * K + 4⟩ [fm]⊢*
              ⟨4 * K + 6, 0, 0, 0, 4 * K + 5⟩ := by
      rw [show K + 3 = (K + 2) + 1 from by ring]
      have h := r3r2r2_chain (K + 1) (K + 2) (3 * K + 4)
      apply stepStar_trans h
      rw [show (K + 2) + 3 * (K + 1) + 1 = 4 * K + 6 from by ring,
          show (3 * K + 4) + (K + 1) = 4 * K + 5 from by ring]
      finish
    rw [show 2 * n + 4 = 4 * K + 6 from by omega,
        show 2 * n + 3 = 4 * K + 5 from by omega]
    exact stepStar_stepPlus_stepPlus p1
      (stepPlus_stepStar_stepPlus p2 (stepStar_trans p3 (stepStar_trans p4 p5)))
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  intro n
  refine ⟨2 * n + 2, ?_⟩
  show ⟨n + 2, 0, 0, 0, n + 1⟩ [fm]⊢⁺
    ⟨(2 * n + 2) + 2, 0, 0, 0, (2 * n + 2) + 1⟩
  rw [show (2 * n + 2) + 2 = 2 * n + 4 from by ring,
      show (2 * n + 2) + 1 = 2 * n + 3 from by ring]
  exact main_trans n

end Sz22_2003_unofficial_1665
