...
/-!
# sz22_2003_unofficial #221: [10/3, 99/35, 1/5, 49/2, 1/77, 5/7]
...
namespace Sz22_2003_unofficial_221
...
theorem r2r1_loop : ∀ k a c e, ⟨a, 0, c + 1, k, e⟩ [fm]⊢* ⟨a + 2 * k, 0, c + 1 + k, 0, e + k⟩ := by
  ...
  | succ k ih =>
    ...
    step fm
    step fm
    apply stepStar_trans (ih (a + 2) (c + 1) (e + 1))
    ring_nf; finish

theorem r3_drain : ∀ c a e, ⟨a, 0, c, 0, e⟩ [fm]⊢* ⟨a, 0, 0, 0, e⟩ := by
...
theorem main_cycle (n : ℕ) : ⟨0, 0, 0, n + 2, 0⟩ [fm]⊢⁺ ⟨0, 0, 0, 3 * n + 3, 0⟩ := by
  ...
  · show fm ⟨0, 0, 0, (n + 1) + 1, 0⟩ = some ⟨0, 0, 1, n + 1, 0⟩
    unfold fm; simp
  apply stepStar_trans (r2r1_loop (n + 1) 0 0 0)
  simp only [Nat.zero_add]
  apply stepStar_trans (r3_drain (1 + (n + 1)) (2 * (n + 1)) (n + 1))
  apply stepStar_trans (r4_drain (2 * (n + 1)) 0 (n + 1))
  simp only [Nat.zero_add]
