...
/-!
# sz22_2003_unofficial #988: [4/15, 33/14, 65/2, 7/11, 44/13]
...
namespace Sz22_2003_unofficial_988
...
theorem main_trans (n f : ℕ) :
    ⟨(0 : ℕ), 0, 2 * n + 1, 0, n, f + 1⟩ [fm]⊢⁺ ⟨0, 0, 2 * n + 3, 0, n + 1, f + n + 2⟩ := by
  rcases n with _ | n
  · -- n = 0
  ...
  · -- n ≥ 1
    ...
    have h2 : ⟨(0 : ℕ), 0, 2 * (n + 1) + 1, n + 1, 0, f + 1⟩ [fm]⊢⁺
              ⟨2, 0, 2 * (n + 1) + 1, n + 1, 1, f⟩ := by
      step fm; finish
    have h3 : ⟨2, (0 : ℕ), 2 * (n + 1) + 1, n + 1, 1, f⟩ [fm]⊢*
              ⟨n + 3, 0, n + 2, 0, n + 2, f⟩ := by
      have := r2r1_chain (n + 1) 1 (n + 2) 0 1 f
      simp only [show (n + 2) + (n + 1) = 2 * (n + 1) + 1 from by ring,
                  show 0 + (n + 1) = n + 1 from by ring,
                  show 1 + (n + 1) = n + 2 from by ring] at this
      exact this
    have h4 : ⟨n + 3, (0 : ℕ), n + 2, 0, n + 2, f⟩ [fm]⊢*
              ⟨0, 0, 2 * n + 5, 0, n + 2, f + n + 3⟩ := by
      have := r3_drain (n + 3) (n + 2) (n + 2) f
      rw [show (n + 2) + (n + 3) = 2 * n + 5 from by ring,
          show f + (n + 3) = f + n + 3 from by ring] at this
      exact this
    show ⟨(0 : ℕ), 0, 2 * (n + 1) + 1, 0, n + 1, f + 1⟩ [fm]⊢⁺
         ⟨0, 0, 2 * (n + 1) + 3, 0, (n + 1) + 1, f + (n + 1) + 2⟩
...
theorem nonhalt : ¬halts fm c₀ := by
  ...
  intro ⟨n, f⟩
  refine ⟨⟨n + 1, f + n + 1⟩, ?_⟩
  show ⟨(0 : ℕ), 0, 2 * n + 1, 0, n, f + 1⟩ [fm]⊢⁺ ⟨0, 0, 2 * (n + 1) + 1, 0, n + 1, f + n + 1 + 1⟩
  rw [show f + n + 1 + 1 = f + n + 2 from by ring,
      show 2 * (n + 1) + 1 = 2 * n + 3 from by ring]
  exact main_trans n f

end Sz22_2003_unofficial_988
