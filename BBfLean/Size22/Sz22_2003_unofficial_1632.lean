...
/-!
# sz22_2003_unofficial #1632: [77/15, 26/3, 9/91, 5/11, 33/2]
...
namespace Sz22_2003_unofficial_1632
...
theorem main_trans_n_odd (m A : ℕ) :
    ⟨A + 1, 0, 0, 0, 2 * m + 1, 2 * m + 1⟩ [fm]⊢⁺
    ⟨A + 2 * m + 2, 0, 0, 0, 2 * m + 2, 2 * m + 2⟩ := by
  -- Phase 1: R4 drain e to c
  have p1 : ⟨A + 1, 0, 0, 0, 2 * m + 1, 2 * m + 1⟩ [fm]⊢*
      ⟨A + 1, 0, 2 * m + 1, 0, 0, 2 * m + 1⟩ := by
    have := e_to_c (2 * m + 1) (A + 1) 0 (2 * m + 1)
    simp only [Nat.zero_add] at this; exact this
  -- Phase 2: R5, R1, R3
  have p2 : ⟨A + 1, 0, 2 * m + 1, 0, 0, 2 * m + 1⟩ [fm]⊢⁺
      ⟨A, 2, 2 * m, 0, 2, 2 * m⟩ := by
  ...
  have p3 : ⟨A, 2, 2 * m, 0, 2, 2 * m⟩ [fm]⊢*
    ...
    exact h
  -- Phase 4: R2, R2
  have p4 : ⟨A, 2, 0, m, 2 * m + 2, m⟩ [fm]⊢*
      ⟨A + 2, 0, 0, m, 2 * m + 2, m + 2⟩ := by
    step fm; step fm; ring_nf; finish
  -- Phase 5: drain
  have p5 : ⟨A + 2, 0, 0, m, 2 * m + 2, m + 2⟩ [fm]⊢*
      ⟨A + 2 * m + 2, 0, 0, 0, 2 * m + 2, 2 * m + 2⟩ := by
    have h := drain_chain m (A + 2) 0 (2 * m + 2) (m + 1)
...
theorem main_trans_n_even (m A : ℕ) :
    ⟨A + 1, 0, 0, 0, 2 * m + 2, 2 * m + 2⟩ [fm]⊢⁺
    ⟨A + 2 * m + 3, 0, 0, 0, 2 * m + 3, 2 * m + 3⟩ := by
  -- Phase 1: R4 drain e to c
  have p1 : ⟨A + 1, 0, 0, 0, 2 * m + 2, 2 * m + 2⟩ [fm]⊢*
      ⟨A + 1, 0, 2 * m + 2, 0, 0, 2 * m + 2⟩ := by
    have := e_to_c (2 * m + 2) (A + 1) 0 (2 * m + 2)
    simp only [Nat.zero_add] at this; exact this
  -- Phase 2: R5, R1, R3
  have p2 : ⟨A + 1, 0, 2 * m + 2, 0, 0, 2 * m + 2⟩ [fm]⊢⁺
      ⟨A, 2, 2 * m + 1, 0, 2, 2 * m + 1⟩ := by
    rw [show A + 1 = A + 1 from rfl,
        show 2 * m + 2 = (2 * m + 1) + 1 from by ring,
        show 2 * m + 2 = (2 * m + 1) + 1 from by ring]
    step fm; step fm; step fm; ring_nf; finish
  -- Phase 3: R1R1R3 chain
  have p3 : ⟨A, 2, 2 * m + 1, 0, 2, 2 * m + 1⟩ [fm]⊢*
      ⟨A, 2, 1, m, 2 * m + 2, m + 1⟩ := by
    ...
    exact h
  -- Phase 3b: R1, R2
  have p3b : ⟨A, 2, 1, m, 2 * m + 2, m + 1⟩ [fm]⊢*
      ⟨A + 1, 0, 0, m + 1, 2 * m + 3, m + 2⟩ := by
    ...
    ring_nf; finish
  -- Phase 4: drain
  have p4 : ⟨A + 1, 0, 0, m + 1, 2 * m + 3, m + 2⟩ [fm]⊢*
      ⟨A + 2 * m + 3, 0, 0, 0, 2 * m + 3, 2 * m + 3⟩ := by
    have h := drain_chain (m + 1) (A + 1) 0 (2 * m + 3) (m + 1)
    rw [show 0 + (m + 1) = m + 1 from by ring,
        show m + 1 + 1 = m + 2 from by ring,
        show A + 1 + 2 * (m + 1) = A + 2 * m + 3 from by ring,
        show m + 1 + (m + 1) + 1 = 2 * m + 3 from by ring] at h
    exact h
  exact stepStar_stepPlus_stepPlus p1
    (stepPlus_stepStar_stepPlus p2
...
theorem main_trans (A N : ℕ) :
    ⟨A + 1, 0, 0, 0, N, N⟩ [fm]⊢⁺ ⟨A + N + 1, 0, 0, 0, N + 1, N + 1⟩ := by
  rcases N with _ | N
  · -- N = 0: (A+1, 0, 0, 0, 0, 0) →⁺ (A+1, 0, 0, 0, 1, 1)
    step fm; step fm; ring_nf; finish
