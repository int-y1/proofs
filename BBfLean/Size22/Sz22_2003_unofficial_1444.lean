...
/-!
# sz22_2003_unofficial #1444: [7/15, 242/3, 9/77, 5/11, 63/2]
...
namespace Sz22_2003_unofficial_1444
...
theorem e_to_c : ∀ k c, (⟨a, 0, c, 0, k + e⟩ : Q) [fm]⊢* ⟨a, 0, c + k, 0, e⟩ := by
  intro k; induction k with
  | zero => intro c; ring_nf; finish
  | succ k ih =>
    intro c; rw [show k + 1 + e = k + (e + 1) from by ring]
    step fm
    apply stepStar_trans (ih (c + 1)); ring_nf; finish

-- Drain chain: (R5, R1, R1) repeated, consuming a and c, producing d
...
theorem main_even (a m : ℕ) (ha : a ≥ m + 2) :
  ...
  have h2 : (⟨a, 0, 2 * (m + 1), 0, 0⟩ : Q) [fm]⊢* ⟨a - (m + 1), 0, 0, 3 * (m + 1), 0⟩ := by
    ...
    rw [show a - (m + 1) + (m + 1) = a from by omega,
        show 0 + 2 * (m + 1) = 2 * (m + 1) from by ring,
        show 0 + 3 * (m + 1) = 3 * (m + 1) from by ring] at this
    exact this
  -- Phase 3: R5, R2, R2
  have h3 : (⟨a - (m + 1), 0, 0, 3 * (m + 1), 0⟩ : Q) [fm]⊢* ⟨a - m, 0, 0, 3 * m + 4, 4⟩ := by
    rw [show a - (m + 1) = (a - (m + 2)) + 1 from by omega,
        show 3 * (m + 1) = 3 * m + 3 from by ring]
    step fm; step fm; step fm
    rw [show a - (m + 2) + 1 + 1 = a - m from by omega]; ring_nf; finish
  -- Phase 4: r3r2r2 chain
  have h4 : (⟨a - m, 0, 0, 3 * m + 4, 4⟩ : Q) [fm]⊢* ⟨a + 5 * m + 8, 0, 0, 0, 9 * m + 16⟩ := by
    rw [show 3 * m + 4 = 0 + (3 * m + 4) from by ring,
        show (4 : ℕ) = 3 + 1 from rfl]
    apply stepStar_trans (r3r2r2 (3 * m + 4) (a - m) 0 3)
    rw [show a - m + 2 * (3 * m + 4) = a + 5 * m + 8 from by omega,
        show 3 + 3 * (3 * m + 4) + 1 = 9 * m + 16 from by ring]
    finish
  exact stepPlus_stepStar_stepPlus h1 (stepStar_trans h2 (stepStar_trans h3 h4))

theorem nonhalt : ¬halts fm c₀ := by
  apply stepStar_not_halts_not_halts (c₂ := ⟨4, 0, 0, 0, 7⟩) (by execute fm 6)
